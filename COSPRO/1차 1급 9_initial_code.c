...
int solution(int *recordA, int recordA_len, int *recordB, int recordB_len) {
  ...
  for (int i = 0; i < recordA_len; i++) {
    if (recordA[i] == recordB[i])
      continue;
    else if (recordA[i] == func(recordB[i]))  // a가 이긴경우
      cnt = cnt + 3;
    else  // a가 진경우
      cnt = (cnt == 0) ? 0 : cnt - 1;
  }
...
// The following is main function to output testcase. The main function is
// correct and you shall correct solution function.
int main() {
